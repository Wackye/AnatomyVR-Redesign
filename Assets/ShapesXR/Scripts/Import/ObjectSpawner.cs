...
namespace ShapesXr
{
    public class ObjectSpawner
    {
        ...
        public void SpawnAllObjects()
        {
            ...
            foreach (var objectId in _allObjectIds)
            {
                ...
                RemoveDummyComponents(spawnedObject);
            }

            _spaceDescriptor.ActiveStage = 0;
        }
        ...
        private void SpawnViewpoints()
        {
            ...
            {
                var viewpointObject = SpawnObject(viewpointIds[i], viewpointsRoot.transform);
                viewpointObject.name = $"Viewpoint {i + 1}";

                _specialObjectIds.Add(viewpointIds[i]);
            }
        }
        ...
        private void SpawnStageContainers()
        {
            ...
            {
                var stageObject = SpawnObject(stageIds[i], stagesRoot.transform);
                stageObject.name = $"Stage {i + 1}";

                _specialObjectIds.Add(stageIds[i]);
        ...
        private void InitializeObject(Guid objectId)
        {
            ...
            var reactors = objectContainer.GetComponentsInChildren<PropertyReactorComponent>(true);
        ...
        private void RemoveDummyComponents(GameObject go)
        {
            var componentsToRemove = go.GetComponentsInChildren<RemoveAfterImportBehaviour>(true);
